...
public class RESTClientTest {
	  ...
	  private static URI getBaseURI() {
		  	//Sin SSL:
		    //return UriBuilder.fromUri("http://localhost:8480/sl07_restful").build();
	  ...
	  //Si /rest/orders/1 acepta petición pidiendo respuesta en MediaType.APPLICATION_JSON solicitamos esa orden en JSON 
	    ...
	    //Nos creamos un objeto orden similar al order 0 por defecto pero con algunos cambios, algunos de ellos erróneos
		 ...
		 Response response;
		  ...
		  order.setToppings("bacon cheese onion mushroom");
	    ...
	    order.setId((long)0);
	    response = target.path("rest").path("orders").path("1").request(MediaType.APPLICATION_JSON).put(Entity.entity(order,MediaType.APPLICATION_JSON),Response.class);
		System.out.println(response.getStatus());
