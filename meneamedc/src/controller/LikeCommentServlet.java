...
public class LikeCommentServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Comment c = cDao.get(id);
		int new_likes = c.getLikes() + 1;
		c.setLikes(new_likes);
		cDao.save(c);
		logger.info("Like comment");
		long news= Long.parseLong(request.getParameter("new"));
		response.sendRedirect("ComentarioNoticia?id="+news);
